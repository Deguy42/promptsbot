    ...
    public class QnABot : DialogBot<QnADialog>
    {
        ...
        protected override async Task OnMembersAddedAsync(IList<ChannelAccount> membersAdded, ITurnContext<IConversationUpdateActivity> turnContext, CancellationToken cancellationToken)
        {
            foreach (var member in membersAdded)
            {
                ...
                {
                    await turnContext.SendActivityAsync(MessageFactory.Text($"Welcome to North Herts Help Bot"), cancellationToken);
                    await turnContext.SendActivityAsync(MessageFactory.Text($"You can search for the following: New Starters, MiCollab"), cancellationToken);
